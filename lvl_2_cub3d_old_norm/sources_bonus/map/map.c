...
/*   Updated: 2021/03/15 16:03:19 by tisantos         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../cub3d_bonus.h"

static void	get_sprite_locations(t_mlx *mlx, int x, int y, int a)
{
	mlx->worldmap[y][x] = 0;
	mlx->sprite_x[mlx->num_sprites] = y + 0.5;
	mlx->sprite_y[mlx->num_sprites] = x + 0.5;
	mlx->sprite_texture[mlx->num_sprites] = a;
	mlx->num_sprites++;
}
...
static void	add_map_to_game2(t_parse *parse, t_mlx *mlx, int x, int y)
{
		...
		|| parse->map_string[y - 1][x - 1] == 'W')
	{
		...
		get_sprite_locations(mlx, x, y, 10);
	else if (parse->map_string[y - 1][x - 1] == '3')
		get_sprite_locations(mlx, x, y, 8);
	else
